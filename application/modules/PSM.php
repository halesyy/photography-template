<?php
  class PSM  {
    /*
    | @NOTE https://github.com/halesyy/psm
    | @AUTHOR Jack Hales
    */
    /*
    ...
    */
      ...
      */
        ...
        public function artist($reference) {
          if ($this->hasdata("SELECT reference FROM artists WHERE reference = :ref", [':ref' => $reference])) {
            // refernece is real
            $set = $this->set("SELECT * FROM artists WHERE reference = :ref", [':ref' => $reference]);
            return $set;
          } else {
            // no reference
            return false;
